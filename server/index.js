...
const sessionNotes = new Map(); // persists after session is deleted; null = generating
...
async function generateNotes(sessionId, transcriptEntries, languageA, languageB) {
  try {
    const formatted = transcriptEntries
      .map((e) =>
        `[${e.direction === "A→B" ? `Party A (${languageA})` : `Party B (${languageB})`}]: ${e.original}`
      )
      .join("\n");
    ...
    const response = await fetch("https://api.openai.com/v1/chat/completions", {
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            role: "system",
            content: `You are a professional call notes assistant. Analyze this phone call transcript (translated in real-time between ${languageA} and ${languageB}) and produce structured notes in JSON.
...
All output must be in English. Return empty arrays if there are no key points, action items, or topics.`,
          },
          {
            role: "user",
